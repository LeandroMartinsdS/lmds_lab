...
from models.peakmodels import skew_super_gaussian

PrepResult = Tuple[Callable, list, Tuple[list, list]]  # (func, p0, bounds)

def _prep_skew_super_gaussian(y, center_x, width_est, C_guess) -> PrepResult:
    func = skew_super_gaussian
    ...
    m0 = 2.0
    skew0 = 0.0
    p0 = [A0, center_x, sigma0, m0, skew0, float(C_guess)]

    # Reasonable bounds to keep search well-posed
    lower = [0.0,   -np.inf, 1e-12,  1.0, -2.0, -np.inf]
    upper = [np.inf, np.inf,  np.inf, 10.0, 2.0,  np.inf]
    bounds = (lower, upper)
    return func, p0, bounds

_MODEL_PREP: Dict[str, Callable] = {
    "skew_super_gaussian": _prep_skew_super_gaussian
}
...
def optimize_peak(x: np.ndarray,
                  y: np.ndarray,
                  model: str = "skew_super_gaussian",
    ...
    widths, _, left, right = peak_widths(y, peaks, rel_height=0.5)
    ...
    window_mask = (x > peak_x - window) & (x < peak_x + window)
    if np.count_nonzero(window_mask) < 10:
        # Expand once if too few points
        window_mask = (x > peak_x - 2 * window) & (x < peak_x + 2 * window)
    ...
    try:
        func, p0, bounds = _MODEL_PREP[model](y, peak_x, width_est, C_guess)
    except KeyError:
    ...
    def func_s1(x_in, A, mu, sigma, C):
        ...
        return func(x_in, A, mu, sigma, m_fixed, skew_fixed, C)

    # Build p0 and bounds for the reduced parameter set [A, mu, sigma, C]
    p0_s1 = [p0[0], p0[1], p0[2], p0[5]]
    bounds_s1 = ([bounds[0][0], bounds[0][1], bounds[0][2], bounds[0][5]],
                 [bounds[1][0], bounds[1][1], bounds[1][2], bounds[1][5]])
    ...
    # Compose starting point for Stage 2: [A, mu, sigma, m, skew, C]
    p0_s2 = [popt_s1[0], popt_s1[1], popt_s1[2], m_fixed, skew_fixed, popt_s1[3]]
